...
log = logging.getLogger("adar.telemetry")
_configured = False
_provider = None
...
def safe_attributes(values: dict[str, Any] | None, *, prefix: str = "adar") -> dict[str, Any]:
    output: dict[str, Any] = {}
    for key, value in (values or {}).items():
        ...
        if any(token in lowered for token in ("authorization", "password", "secret", "api_key", "apikey", "access_token", "refresh_token")):
            ...
            continue
        if any(token in lowered for token in ("prompt", "question", "content", "response", "message_text")) and not _capture_content():
            rendered = str(value)
...
def traced_span(name: str, *, attributes: dict[str, Any] | None = None, kind=None) -> Iterator[Any]:
    if not telemetry_enabled():
        yield None
        return
    tracer = trace.get_tracer("adar.core")
